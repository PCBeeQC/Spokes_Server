...
public static class ChatPreviewFormatter
{
    ...
    public static string FormatPreviewContent(string? rawContent, List<ChatAttachment>? attachments = null)
    {
    ...
    public static string FormatAndTruncatePreview(string? rawContent, List<ChatAttachment>? attachments = null, int maxLength = 30)
    {
        var formatted = FormatPreviewContent(rawContent, attachments);
        if (string.IsNullOrEmpty(formatted)) return "";
        
        return formatted.Length > maxLength ? formatted[..(maxLength - 3)] + "..." : formatted;
    }
}
