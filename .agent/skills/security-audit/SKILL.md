---
name: security-audit
description: Security guidance and vulnerability review for codebases, APIs, services, CLI tools, libraries, and daemons. Use for security questions, focused reviews, vulnerability research, security audits, or pen tests. Run the complete workflow only for explicit codebase audit or pen-test requests, full/comprehensive/end-to-end reviews, or requested report artifacts. Based on the Cloudflare security-audit-skill harness (MIT).
---
...
This skill uses generic terminology mapped to the Antigravity platform:

- **Parent** is the orchestrating agent that coordinates the run and owns shared state — this is you, the agent running this skill.
- **Task tool** is `invoke_subagent` — the platform's delegation mechanism for launching isolated agents.
- **`research` agent** is a `research` subagent (read-only tools for source exploration and factual verification).
- **`general` agent** is a `self` subagent (full tools including write access for broad investigation and bounded local execution).
- **`subagent_type:`** in a heading names which of these two subagent roles runs that work.

Use `invoke_subagent` with `TypeName: "research"` for `research` agents and `TypeName: "self"` for `general` agents. Preserve role, write-isolation, prompt, and independence boundaries.

## Target-specific context

When auditing the Spokes codebase, read [spokes-context.md](spokes-context.md) during Phase 1 reconnaissance to accelerate architecture mapping. This file provides Spokes-specific architecture context (authentication flows, data persistence, entry surfaces, trust boundaries) that supplements the reconnaissance agents' source analysis.
...
- **Output directory**: a new writable directory outside the target, defaulting to `~/security-audits/<repo-name>/run-<N>`, where `<N>` is the next unused integer. Use a directory inside the target only when the user explicitly selects it and the parent verifies that version control ignores the whole directory. Otherwise stop and request an external path.
